...
// ── the cues, which are content and not a description of a file ────────────
//
// A cue track is the third thing in a document that cannot be re-derived from
// anything — beside the clips and the graph — because the file a track may have
// been forked out of has stopped being what renders. So three things have to
// survive a write and a read, and each of them is a way work is lost if it does
// not: the **words**, obviously; each cue's **dialogue line**, which is where an
// ASS track's styling lives and which a document holding only the words would
// flatten on the first save; and the track's **id**, because a subtitle row on
// the Write stage says `cues:3` and an open that renumbered would point it at
...
console.log('\nthe cues it holds');
{
    ...
    pump(150);
    A.open(media);
    waitFor('a clip', () => A.project.clips.length === 1);
    pump(200);

    const track = A.cues.makeCueTrack({
        name: 'Dialogue',
        header: '[Script Info]\nScriptType: v4.00+\n\n[Events]\nFormat: Layer, Start, ' +
                'End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n',
        cues: [{ start: 1, end: 2.5, text: 'first', raw: '0,0,Alt,,10,10,20,,{\\i1}first' },
               { start: 4, end: 5, text: 'second' }],
    });
    // A second one, so that "the ids come back" is a fact rather than a
    // coincidence about the number 1.
    const spare = A.cues.makeCueTrack({ name: 'Notes' });
    A.cues.cuesChanged();
    pump(120);

    const before = { id: track.id, spare: spare.id };
    const blob = doc.snapshot();
    same(blob.subtitles.length, 2, 'a document carries every cue track');
    same(blob.subtitles[0].cues[0].raw, '0,0,Alt,,10,10,20,,{\\i1}first',
         'with each cue’s dialogue line beside its words — which is where the styling is');

    doc.open(JSON.parse(JSON.stringify(blob)));
    pump(200);
    same(A.cues.cueTracks.length, 2, 'and puts them back');
    same(A.cues.cueTracks[0].id, before.id, 'keeping the ids, which a stream row is written against');
    same(A.cues.cueTracks[1].id, before.spare, 'both of them');
    same(A.cues.cueTracks[0].cues[0].text, 'first', 'the words');
    ok(A.cues.hasOverrides(A.cues.cueTracks[0].cues[0]),
       'and the override codes, so a styled cue is still styled after a save and a reopen');
    ok(A.cues.cueFileText(A.cues.cueTracks[0], 0, 10).indexOf(',Alt,,10,10,20,,{\\i1}first') >= 0,
       'which is what comes out when the render writes the file');

    // A fresh id counter is what a renumbering document would produce, so the
    // check is that the next track made after an open does not collide with one
    // the document already named.
    const next = A.cues.makeCueTrack({ name: 'After' });
    ok(next.id !== before.id && next.id !== before.spare,
       `a track made after an open takes an id the document has not used (${next.id})`);
    A.cues.removeCueTrack(next);

    // Hand-edited, and the reader has to survive it: a document is a text file
    // people open. Same rule as every other reader here.
    const rough = JSON.parse(JSON.stringify(blob));
    rough.subtitles = [{ id: 'nonsense', cues: [{ start: 'x', end: -4, text: 5 },
                                                null, 'not a cue'] }, 7, null];
    doc.open(rough);
    pump(150);
    same(A.cues.cueTracks.length, 1, 'a hand-edited subtitles list opens as what it describes');
    same(A.cues.cueTracks[0].cues.length, 1, 'with the entries that are not cues left out');
    same(`${A.cues.cueTracks[0].cues[0].start}/${A.cues.cueTracks[0].cues[0].end}`, '0/0',
         'and a cue whose times are not numbers clamped rather than drawn at NaN');

    doc.reset();
    pump(150);
    same(A.cues.cueTracks.length, 0, 'New takes the cues with it, like everything else in the edit');
}
